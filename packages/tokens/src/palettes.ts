/**
 * Tailwind-style Color Palettes
 *
 * Pre-defined color palettes with shades from 50-950.
 * These match Tailwind CSS v3 colors for familiarity.
 */
...
// Zinc - Cool gray, slightly warmer than slate
...
// Helper to get CSS variable for a palette color
export function paletteVar(name: PaletteName, shade: PaletteShade): string {
  return `var(--st-${name}-${shade})`;
}
...
export function generatePalettesCss(): string {
  const lines: string[] = [
    "/**",
    " * @staple-css/tokens - Color Palettes",
    " *",
    " * Tailwind-compatible color palettes with shades 50-950.",
    " */",
    "",
    ":root {",
  ];
  ...
  for (const [name, palette] of Object.entries(palettes)) {
    ...
    for (const [shade, color] of Object.entries(palette)) {
      lines.push(`  --st-${name}-${shade}: ${color};`);
    }
    lines.push("");
  }
